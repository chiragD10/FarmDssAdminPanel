...
const AddFilesPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchLessonFiles = async () => {
            ...
            if (token && brandId) {
                try {
                    const response = await axios.post(base_url+`/kbank/crops/technicalnames/brand/files/`, { id: brandId }, config);
                    setBrandFiles(response.data.brandfiles.brandfiles || []);
                    setMediaId(response.data.brandfiles)
                } catch (error) {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        const brand_Id: number | undefined = brandId ? parseInt(brandId) : undefined;
        ...
        try {
            const response = await axios.post(base_url+'/kbank/crops/technicalnames/brand/files/add/', payload, config);
    ...
    return (
        <div className="container mx-auto p-4">
            <div className='text-xcodebblue font-poppins font-semibold mt-4'>{location.state.crop_name} &gt; {location.state.topic_name} &gt; {location.state.subtopic_name} &gt; {location.state.technicalName} &gt; {location.state.product_name}</div>
            <button
            ...
            <div className="overflow-x-auto mt-10">
                <table className="min-w-full leading-normal">
                    ...
                    <tbody>
                        {brandfiles.slice((currentPage - 1) * itemsPerPage, Math.min(currentPage * itemsPerPage - 1, brandfiles.length)).map((lessonfile, index) => (
            ...
            <Pagination entries={brandfiles.length} ataTime={itemsPerPage} currentPage={currentPage} setCurrentPage={setCurrentPage}></Pagination>
